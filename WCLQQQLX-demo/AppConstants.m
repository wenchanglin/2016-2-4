...
+(NSString*)WXAppID {
    return @"wxad1f76157e677741";
}

+(NSString*)WXAppSecret {
    return @"39f2a00d9b81984f20cab6ba5e40bb32";
}
...
+(NSString*)SINAAppKey {
    return @"1436472074";//1436472074
}
+(void)YeMianSetStatus:(YeMianStatus)yemianstatus
{
    switch (yemianstatus) {
        case FirstYemian:
                NSLog(@"首页");
    ...
    ((AppDelegate*)[[UIApplication sharedApplication] delegate]).yemian = yemianstatus;
}
+(YeMianStatus)yemianStatus
{
      return ((AppDelegate*)[[UIApplication sharedApplication] delegate]).yemian;
}
...
+(NSString*)httpImageHeader {
    if ([NSLocalizedStringCN(@"test", @"") isEqualToString:@"测试"]) {
...
+(NSString*)shareHeader {
    if ([NSLocalizedStringCN(@"test", @"") isEqualToString:@"测试"]) {
        //        return @"http://120.26.37.253/";
        return @"http://upload.cn.greenis-service.com.cn/";
    }
    else {
        //        return @"http://120.26.203.184/";
        return @"http://upload.en.greenis-service.com.cn/";
    }
}
...
+(NSString*)httpVideoHeader {
    if ([NSLocalizedStringCN(@"test", @"") isEqualToString:@"测试"]) {
        //        return @"http://120.26.37.253/";
        return @"http://video.cn.greenis-service.com.cn/";
    }
    else {
        //        return @"http://120.26.203.184/";
        return @"http://video.en.greenis-service.com.cn/";
    }
}
...
+(NSString *)httpChinaAndEnglishForHead
{
    if ([NSLocalizedStringCN(@"test", @"") isEqualToString:@"测试"]) {
        return @"http://image.cn.greenis-service.com.cn";//中文单独访问
    }
    else 
    {
        return @"http://image.en.greenis-service.com.cn";//英文时单独访问
    }
}
...
+(void)totalForChat:(NSInteger)total
{
   ((AppDelegate*)[[UIApplication sharedApplication] delegate]).total = total;
}
+(NSInteger)total
{
    return ((AppDelegate*)[[UIApplication sharedApplication] delegate]).total;
}
...
+(void)saveModelWithKey:(NSString*)key andArray:(NSArray*)array {
    ...
    for(int i = 0; i < [array count]; i++){
        ...
        [jsonString appendString:[((IntroduceDataModel*)[array objectAtIndex:i]) modelToJSONString]];
...
+(NSString*)AppID {
    return @"1067728404";// 编号： 100090201265
...
+(void)notice2ManualRelogin {
    ...
    [ProgressHUD showError:NSLocalizedStringCN(@"notice2ManualRelogin", @"")];
