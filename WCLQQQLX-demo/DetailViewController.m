...
@interface DetailViewController ()<ShareSheetDelegate,MessageInputViewDelegate,MessageManagerFaceViewDelegate,WXApiManagerDelegate, TencentSessionDelegate,UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate,BLESerialComManagerDelegate>
...
@property(assign,nonatomic) int currentStepIndex;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [MobClick endLogPageView:@"detailVC"];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [MobClick beginLogPageView:@"detailVC"];
...
- (void)viewDidLoad {
    ...
    NSArray * arrays = [self.steps componentsSeparatedByString:@"%"];
...
-(void)initFaceView
{
    _faceView = [[MessageManagerFaceView alloc] init];
...
-(void)requestData
{
    ...
    [manager POST:urlString parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if ([[responseObject objectForKey:@"ok"]intValue]==1) {
           ...
           // NSLog(@"responseObjectppp   %@",responseObject);
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UIView * view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 214 + [UIScreen mainScreen].bounds.size.width /800 * 533)];
    [self.view addSubview:view];
    view.backgroundColor = [UIColor whiteColor];
    //主图
    NSString * string = [NSString stringWithFormat:@"%@%@",[AppConstants httpChinaAndEnglishForHead],self.mainimageView];
    //视频图
    UIImageView * videoImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width /800 * 533)];
    [videoImageView sd_setImageWithURL:[NSURL URLWithString:string] placeholderImage:[UIImage imageNamed:@"placeholder.jpg"]];
    ...
    //功效详情
    gongxiaoDetail.font = [UIFont systemFontOfSize:12.0f];
    gongxiaoDetail.textColor = [UIColor colorWithHexString:@"#808080"];
    ...
    //材料文字
    UILabel *caiLiaoLabel = [[UILabel alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 120) / 2, CGRectGetMaxY(gongxiaoDetail.frame) + 50, 120, 30)];
    caiLiaoLabel.text = NSLocalizedStringCN(@"cailiao", @"");
    caiLiaoLabel.textColor = [UIColor colorWithHexString:@"#333333"];
    ...
    //    做法
    UILabel *practice = [[UILabel alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 120) / 2, CGRectGetMaxY(aLabel.frame) + 40, 120, 40)];
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    ...
    {
        //        670 - 210 - 246 + [UIScreen mainScreen].bounds.size.width /800 * 533
        if (aArray.count==1||aArray.count==2) {
            return dongTaiRect.size.height+50+40+214 + [UIScreen mainScreen].bounds.size.width /800 * 533;
        }
        else if (aArray.count==3||aArray.count==4) {
            return dongTaiRect.size.height+50+40+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+10 ;
        }
        else if (aArray.count==5||aArray.count==6) {
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+20  ;
        }
        else if (aArray.count==7||aArray.count==8) {
            
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+60  ;
        }
        else if (aArray.count==9||aArray.count==10) {
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+95 ;
        }
        else if(aArray.count==11||aArray.count ==12)
        {
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+135  ;
        }
        else if (aArray.count==13||aArray.count==14)
        {
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+195;
        }
        else
        {
            return dongTaiRect.size.height+50+40+25+214 + [UIScreen mainScreen].bounds.size.width /800 * 533+195;
...
-(void)didSelectIndex:(NSInteger)index{
    ...
    switch (index) {
        ...
        case 2: // 2 微信朋友圈
        {
            ...
            // ThumbImage 超过10K 会分享失败
            [WXApiRequestHandler sendLinkURL:linkUrl TagName:@"SmartKitchen" Title:self.name  Description:self.detailStr
                                              ThumbImage:[UIImage imageNamed:@"icon.png"]  InScene:WXSceneTimeline];
        }
            break;
        case 3:// 3 QQ好友
        {
...
-(void)bleSerialComManager:(BLESerialComManager *)bleSerialComManager didDataReceivedOnPort:(BLEPort *)port withLength:(unsigned int)length{
    ...
    if ([data hasPrefix:@"SBT:B90400"]) {
        ...
        if([data hasPrefix:@"SBT:B9040003"])
        {
            NSLog(@"杯子或杯盖被拿走机器过流保护机器过热保护机器堵转保护电源线拔掉了 (仅FGR8800 FGR8840 无刷机型才支持这");
    ...
    if ([[BTConstants BTRequestType] isEqualToString:@"C7"]) {
        ...
        if ([data length] > 12) {
            if ([[data substringWithRange:NSMakeRange(0, 12)] isEqualToString:@"SBT:BF02C200"]) {
                ...
                [MobClick event:@"downloadcaipu" label:self.name];
                [BTConstants BTSetStatus:BTStatusReady];
