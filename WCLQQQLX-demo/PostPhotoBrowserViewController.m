...
//    [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:UIStatusBarAnimationSlide];
   // [[UIApplication sharedApplication] setStatusBarHidden:TRUE];
    ...
    self.navigationItem.title = NSLocalizedStringCN(@"tupianliulan", @"");
    ...
    if (_isUrl) {
        if (_images.count==2) {
            ...
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -3;
        }
        else if (_images.count==4) {
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -4;
        }
        ...
        {
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -5;
        }
        else if (_images.count==6) {
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -6;
        }
        else if (_images.count==7)
        {
            _pageControl.numberOfPages = [_images count];
            _pageControl.currentPage = _index-7;
            
        }
        else if (_images.count==8)
        {
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -8;
        }
        else if (_images.count==9)
        {
            _pageControl.numberOfPages = [_images count] ;
            _pageControl.currentPage = _index -9;
        }
...
//        subScrollView.contentSize = CGSizeMake(360, 460);
        ...
        if (_isUrl) {
            ...
            if ([[_images objectAtIndex:i] hasPrefix:@"http:"]) {
                ...
                imageUrl = [NSString stringWithFormat:@"%@%@", [AppConstants httpChinaAndEnglishForHead], [_images objectAtIndex:i]];
            }
...
#pragma mark - 跟上面的[[UIApplication sharedApplication] setStatusBarHidden:TRUE];一起用 return yes
