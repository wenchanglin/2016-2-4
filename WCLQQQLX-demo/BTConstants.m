...
+ (UIImage*)BTConnectedMachineImage:(NSString*)machineName {
    ...
    else if ([machineName hasSuffix:@"8880S"]) {
        return [UIImage imageNamed:@"device8880S.jpg"];
    }
    else if ([machineName hasSuffix:@"8880T"]) {
        return [UIImage imageNamed:@"device8880T.jpg"];
    }
    else if ([machineName hasSuffix:@"8880R"]) {
        return [UIImage imageNamed:@"deviceG-8880R.jpg"];
    }
    else if ([machineName hasSuffix:@"8890"]) {
        return [UIImage imageNamed:@"deviceG-8890.jpg"];
    }
    
    else {
        return [UIImage imageNamed:@"logo.jpg"];
    }
}
...
+ (BTMachineStatus)BTStatus {
    return ((AppDelegate*)[[UIApplication sharedApplication] delegate]).BTStatus;
}
...
+ (NSString*)BTMachineCurrnetMode:(NSString*)str {
    ...
    if ([str hasPrefix:@"SBT:B9040001"]) {
        result = NSLocalizedStringCN(@"shougongmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040002"]) {
        result = NSLocalizedStringCN(@"tongsuomoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040003"]) {
        result = NSLocalizedStringCN(@"miantuanmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040004"]) {
        result = NSLocalizedStringCN(@"shuguomoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040005"]) {
        result = NSLocalizedStringCN(@"mofenmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040006"]) {
        result = NSLocalizedStringCN(@"shabingmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040007"]) {
        result = NSLocalizedStringCN(@"jiangliaomoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040008"]) {
        result = NSLocalizedStringCN(@"doujiangmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040009"]) {
        result = NSLocalizedStringCN(@"bingkuaimoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B904000A"]) {
        result = NSLocalizedStringCN(@"diandongmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040010"]) {
        result = NSLocalizedStringCN(@"xiumianmoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040020"]) {
        result = NSLocalizedStringCN(@"lanyamoshi", @"");
    }
    else if ([str hasPrefix:@"SBT:B9040030"]) {
        result = NSLocalizedStringCN(@"chucuomoshi", @"");
    }
   else  if ([str hasPrefix:@"SBT:B9040050"])
    {
        result = NSLocalizedStringCN(@"diymoshi", "");
    }
    else if ([str hasPrefix:@"SBT:B9040051"])
    {
        result = NSLocalizedStringCN(@"diyluzhi", "");
    }
    else if ([str hasPrefix:@"SBT:B9040081"])
    {
        result = NSLocalizedStringCN(@"bianjiemoshi", "");
    }
    else if([str hasPrefix:@"SBT:B904000D"])
    {
        result = NSLocalizedStringCN(@"naixi", @"");
    }
    else if([str hasPrefix:@"SBT:B904000E"])
    {
        result = NSLocalizedStringCN(@"bingqilin", @"");
    }
    else if([str hasPrefix:@"SBT:B904000B"])
    {
        result = NSLocalizedStringCN(@"lengyin", @"");
    }
    else if([str hasPrefix:@"SBT:B904000C"])
    {
        result = NSLocalizedStringCN(@"reyin", @"");
    }
...
+ (NSString*)B3:(NSString*)content {
        NSString * str =[content substringWithRange:NSMakeRange(9, 1)];
        NSLog(@"B3截取%@",str);
        return str;
//    NSString *str= [NSString stringWithFormat:@"%@B3020%@%@",[BTConstants BTHeadCode]];
}
...
+ (NSString *)B4:(NSString*)content {
    NSString * str;
    NSLog(@"B4拿到的content%@",content);
    if ([content hasPrefix:@"SBT:B40280"]) {
        str = [content substringWithRange:NSMakeRange(0, 10)];
         NSLog(@"B40280%@",str);
    }
    else  if([content  hasPrefix:@"SBT:B4020000"]){
        str = content;
        NSLog(@"B4020000%@",str);
    }
    else
    {
        str = [content substringWithRange:NSMakeRange(7, 2)];
        NSLog(@"B4020%@",str);
...
+ (NSString*)B5:(NSString*)content {
    ...
    if([[content substringWithRange:NSMakeRange(0, 8)] isEqualToString:@"SBT:B507"])
    {
        str = [content substringWithRange:NSMakeRange(8, 7)];
        NSLog(@"软硬件版本:%@",str);
    }
    return str;
}
...
+ (NSString*)B6:(NSString*)content {
    NSString *str;
    if([[content substringWithRange:NSMakeRange(0, 8)] isEqualToString:@"SBT:B608"])
    {
        str = [content substringWithRange:NSMakeRange(8, 8)];
        NSLog(@"出场日期:%@",str);
    }
    return str;
}
