...
//        self.faceSendButton.backgroundColor = [UIColor blackColor];
        ...
        imgView.image = [UIImage imageNamed: @"search_textfield.gif"];
        
        [textView addSubview: imgView];
        
        [textView sendSubviewToBack: imgView];
        textView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
        textView.placeHolder = NSLocalizedStringCN(@"shuodianshenme", @"");
...
//            sender.selected = !sender.selected;
            ...
            if (_faceButtonSelected) {
                NSLog(@"表情被点击");
                [[NSNotificationCenter defaultCenter] postNotificationName:@"Keychange" object:nil userInfo:@{@"1":@"123"}];
                [self.messageInputTextView resignFirstResponder];
